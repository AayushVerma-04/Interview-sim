...
const aiResumeFile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!docResume) {
      return res
        .status(400)
        .json(new ApiResponse(400, "Resume file is empty or not valid"));
    }
...
const aiInterviewWay = asyncHandler(async (req, res) => {
  try {
    ...
    if (!position || !experienceLevel || !numberOfQuestionYouShouldAsk || !sessionId) {
      throw new ApiError(
        400,
        "All required fields (position, experienceLevel, numberOfQuestionYouShouldAsk, sessionId) are required"
      );
    }
...
const aiInterviewStart = async (sessionId, answer) => {
  try {
    ...
    if (!data || Object.keys(data).length === 0) {
      throw new Error(
        `Session not found for sessionId: ${sessionId}. Make sure to create the session first.`
      );
    }

    // Safely parse arrays
    let messages = safeParse(data.messages, []);
    let answers = safeParse(data.answers, []);

    const lowerCaseAnswer = (answer || "").toLowerCase();

    // Only push user messages when it’s a real answer
    if (
      !lowerCaseAnswer.startsWith("//explain") &&
    ...
    const ai = await aiInterview(
      ...
      data.resume || "",
      data.position || "",
      parseInt(data.numberOfQuestionLeft || "0"),
    ...
    if (answer.startsWith("//explain")) {
      ...
      const questionExplain = safeParse(data.aiExplanation, []);
      questionExplain.push({ question: ai.question, explanation: ai.explanation });
      ...
      if (!ai.question.startsWith("Your interview is over")) {
        messages.push({
          role: "ai",
          content: ai.question || "",
          referenceAnswer: ai.referenceAnswer || "",
        });
        ...
        multi.hset(sessionId, "answers", JSON.stringify(answers));
      }

      let questionLeft = Math.max((parseInt(data.numberOfQuestionLeft) || 1) - 1, 0);
      multi.hset(sessionId, "numberOfQuestionLeft", questionLeft);
    }
    ...
    const result = ai.explanation && ai.question ? ai.explanation : ai.question;

    return {
      result,
      numberOfQuestionLeft: Math.max(parseInt(data.numberOfQuestionLeft || 1) - 1, 0),
      audio: audioBuffer,
      filePath,
    };
  } catch (error) {
    console.error("Error in aiInterviewStart:", error);
    throw error;
  }
};
...
const aiInterviewAnalysis = asyncHandler(async (req, res) => {
  try {
    ...
    const aiResponse = await aiAnalysis(
      data.resume,
      data.position,
    ...
    if (!savedSession) {
      throw new ApiError(
        500,
        "Failed to save the interview session to the database."
      );
    }
  ...
  try {
    ...
    if (totalItems === 0) {
      return res
        .status(200)
        .json(
          new ApiResponse(200, responseData, "No history found for this user.")
        );
    }
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, responseData, "History retrieved successfully")
      );
  } catch (error) {
